...
import { useArticles, useFeaturedArticles, useHealth, useContestedClaims } from "./hooks";
...
export default function HomePage() {
  ...
  const { data: contestedRes } = useContestedClaims(5);
  ...
  const contestedClaims = (contestedRes as any)?.claims ?? [];
      ...
      <div className={`${containerClass} mx-auto space-y-12 transition-all duration-300`}>
        ...
        <div className="text-center space-y-4 max-w-2xl mx-auto pt-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-100 text-zinc-700 text-xs font-medium border border-zinc-200">
            ...
            <span>Autonomous Epistemic Corpus · {health?.article_count ?? "1,420"} Verified Entries</span>
          </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <DomainCard
              ...
              category="TECHNOLOGY"
              abstract="Superposition, entanglement, and transmons exploring exponential Hilbert state spaces."
              sourcesCount={31}
              confidence={0.96}
            />
            <DomainCard
              slug="kingdom-of-benin"
              title="Kingdom of Benin"
              category="HISTORY"
              abstract="Pre-colonial West African empire, 16,000km defensive earthworks, and bronze metallurgy."
              sourcesCount={28}
              confidence={0.94}
            />
            <DomainCard
              ...
              category="NATURE & BIO"
              abstract="Solar light conversion, thylakoid Z-scheme, and Calvin cycle carbon fixation mechanics."
              sourcesCount={42}
              confidence={0.98}
            />
            <DomainCard
              ...
              category="PHYSICS"
              abstract="Spacetime curvature, geodesic trajectories, and gravitational wave interferometry."
              sourcesCount={35}
              confidence={0.99}
            />
            <DomainCard
              slug="crispr-gene-drives"
              title="CRISPR Gene Drives"
              category="BIOENGINEERING"
              abstract="Non-Mendelian inheritance engineering, Cas9 homing endonucleases, and ecological resistance."
              sourcesCount={24}
              confidence={0.92}
            />
            <DomainCard
              ...
              abstract="Scaled dot-product self-attention, rotary position embeddings, and autoregressive modeling."
              sourcesCount={50}
              confidence={0.97}
            />
          </div>
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white overflow-hidden shadow-xs">
            ...
            <div className="p-6 sm:p-8 space-y-4">
              <Link
                ...
                className="text-2xl sm:text-3xl font-bold text-zinc-900 hover:text-blue-600 no-underline transition-colors block"
              >
                {(feat as any).title}
              </Link>
              ...
              <div className="pt-3 border-t border-zinc-100 flex items-center justify-between">
                <span className="text-xs text-zinc-500">
                  {((feat as any).citations?.length || 25)} Primary Citations Verified
                </span>
                <Link
                  href={`/article/${(feat as any).slug}`}
