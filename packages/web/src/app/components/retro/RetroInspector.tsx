  ...
  return (
    <div className="border-[2px] bg-white p-3" style={{ borderStyle: "inset", borderColor: "#a09060 #fff9e5 #fff9e5 #a09060" }}>
      <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
          ...
          {Object.keys(cv).length > 0 && (
            <div className="mt-2 space-y-1">
              {Object.entries(cv).slice(0, 6).map(([k, v]) => (
                <div key={k} className="flex items-center gap-2 text-[10px] text-[#555]"><span className="w-16 sm:w-28 truncate">{k}</span><span className="flex-1 h-[5px] bg-[#efe9d5] border border-[#8a7f68]"><span className="block h-full bg-[#0a2a5e]" style={{ width: `${Math.max(0, Math.min(1, Number(v))) * 100}%` }} /></span><span className="tabular-nums">{Number(v).toFixed(2)}</span></div>
              ))}
            </div>
          )}
          {evLinks.length > 0 && (
            <div className="mt-2">
              ...
              <div className="mt-1 space-y-1">
                {evLinks.slice(0, 8).map((e: any, i: number) => {
                  ...
                  return <a key={i} href={other.label?.startsWith("http") ? other.label : undefined} target="_blank" rel="noreferrer" className="block break-words text-[11px] border border-[#c0c0c0] bg-[#fffef6] px-1.5 py-1 no-underline text-black hover:bg-[#fff8dc]"><span className={e.relationship === "supports" ? "text-[#2e7d32] font-bold" : "text-[#a33] font-bold"}>{e.relationship === "supports" ? "+" : "–"}</span> {domainOf(other.label)} <span className="text-[#555]">• {other.chain_of_custody ?? "?"} • {other.accessibility ?? ""}</span></a>;
                })}
              </div>
            </div>
          )}
        ...
        <div className="text-[9px] text-[#555] max-w-none sm:max-w-[150px] border-t sm:border-t-0 border-l-0 sm:border-l pt-2 sm:pt-0 sm:pl-2 shrink-0 w-full sm:w-auto">
