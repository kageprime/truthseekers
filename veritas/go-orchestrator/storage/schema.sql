...
CREATE TABLE IF NOT EXISTS articles (
    ...
    title TEXT NOT NULL,
    abstract TEXT,
    content TEXT, -- Markdown content representation
    blocks JSONB, -- Rich rendering blocks (Block[]) compatible with front-end
    confidence_vector JSONB,
    derived_confidence FLOAT,
...
-- 9. Graph edges (caching relations for quick graph traversal)
CREATE TABLE IF NOT EXISTS graph_edges (
    source TEXT NOT NULL, -- source article slug
    target TEXT NOT NULL, -- target article slug
    relationship TEXT DEFAULT 'related',
    PRIMARY KEY (source, target)
);

-- 10. Map Entries (interactive and static maps)
...
-- 11. Article View counts (for tracking views)
...
-- 12. Job Queue Status (caching job state)
...
-- 13. System Memories / Key-Value Store (replacing mongoose Memory model)
