...
- **Layout Architecture:** Newspaper broadsheet architecture — `Masthead` nameplate (date, time, edition, epistemic weather) + dual rails (`DockedSidebar` section index on the left, `ReadingDesk` on the right ≥xl). Reading pages use the outer-left TOC rail with a centered/left text track; the Research Studio (`/chat`) is a separate full-width workbench.
...
### Broadsheet Canvas + Dual Rails (`AppShell`)
- **Nameplate (`Masthead`)**: Newspaper header across all pages. One compact bar <lg (3.5rem) with hamburger, title, date, view menu, and user orb; two rows ≥lg (5.5rem = `--masthead-h`) with folio strip (date, live time, Vol/No, edition, epistemic weather line: entries, contested, open gaps) and centered wordmark nameplate under a thin rule, closed by a thick-over-thin broadsheet double rule.
- **Left rail (`DockedSidebar`)**: Section index (Encyclopedia / Create / Account). Persistent at ≥lg (defaults to `w-16` icon rail; chevron expands to `w-60` labeled index, pushing content); slide-over drawer <lg with backdrop.
- **Right rail (`ReadingDesk`)**: Appears at ≥xl (`hidden xl:flex w-64 2xl:w-72`). Houses view controls (Width: Focus/Expanded, Align: Left/Center, Type: A/A+/A++) with labeled mono groups; reserved slot for page-context tools. Below xl, reading controls live in the masthead's `Aa` popover.
- **Contents**: `ArticleContents` (`article/ArticleContents.tsx`) — `variant="rail"` (sticky ≥xl, outer-left 13rem track, 0.5rem gap, 0.55rem item padding) + `variant="bar"` (sticky chip strip <xl), driven by `useTocItems` + `useActiveHeading`; anchors via `lib/heading-id.ts`.
...
- **Chrome/page dedupe**: One brand mark per screen via `TruthseekersLogo` / `Masthead` nameplate; no breadcrumb ladder; `PlateHead.tsx` is the single article folio/title/deck/rule masthead.
